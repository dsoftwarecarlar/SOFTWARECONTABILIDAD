...
const APP_PUBLIC_HOST = '192.168.100.147';
...
function app_laravel_public_base_url(): string
{
    $configured = trim((string) (getenv('LARAVEL_PUBLIC_BASE_URL') ?: ''));
    if ($configured !== '') {
        if (preg_match('/^https?:\/\//i', $configured) === 1) {
            return rtrim($configured, '/');
        }

        return app_url($configured);
    }

    return app_url('laravel_app/public');
}

function app_laravel_url(string $relative = ''): string
{
    $base = app_laravel_public_base_url();
    $relative = trim(str_replace('\\', '/', $relative), '/');
    if ($relative === '') {
        return $base;
    }
...
function app_normalize_host(string $host): string
{
    ...
    if (str_contains($host, '://')) {
        ...
        return (string)$parsed['host'] . $port;
    }
...
function app_is_loopback_host(string $host): bool
{
    $host = strtolower(app_host_without_port($host));
    return in_array($host, ['localhost', '127.0.0.1', '::1'], true);
}

function app_detect_public_host(): string
{
    $requestHost = app_normalize_host((string)($_SERVER['HTTP_HOST'] ?? ''));
    if ($requestHost !== '' && !app_is_loopback_host($requestHost)) {
        return $requestHost;
    }

    $serverAddr = app_normalize_host((string)($_SERVER['SERVER_ADDR'] ?? ''));
    if ($serverAddr !== '' && !app_is_loopback_host($serverAddr)) {
        return $serverAddr;
    }
...
function app_public_host(): string
{
    $detectedHost = app_detect_public_host();
    if ($detectedHost !== '') {
        return $detectedHost;
    }

    return app_normalize_host(APP_PUBLIC_HOST);
}
...
{
    return app_laravel_url('downloads/' . rawurlencode($fileName));
}
...
function app_workspaces(): array
{
    $cxpModules = [
        [
            ...
            'url' => app_laravel_url('cxp/modules/accion1'),
            'state' => 'active',
        ],
        [
            'slug' => 'cxp_txt',
            ...
            'url' => app_laravel_url('cxp/modules/accion2'),
            'state' => 'active',
        ],
        [
            'slug' => 'cxp_accion3',
            ...
            'url' => app_laravel_url('cxp/modules/accion3'),
            'state' => 'active',
        ],
        [
            'slug' => 'cxp_accion4',
            ...
            'url' => app_laravel_url('cxp/modules/accion4'),
            'state' => 'active',
        ],
        [
            ...
            'url' => app_laravel_url('cxp/modules/servicios-marcas'),
            'state' => 'active',
        ],
        [
            ...
            'status' => 'Operativo',
            'description' => 'Carga 4 reportes de ventas y 4 reportes de devoluciones por marca para generar la plantilla mensual consolidada de facturacion de repuestos.',
            'url' => app_laravel_url('cxp/modules/repuestos-tytserv'),
            'state' => 'active',
        ],
    ];
        ...
        [
            ...
            'windows' => [
                [
                    ...
                    'url' => app_laravel_url('cxp/windows/libro-compras-aclt'),
                    'modules' => [
                    ...
                    'bundle' => [
                        ...
                        'url' => app_laravel_url('cxp/modules/consolidado-acciones'),
                        'state' => 'active',
                    ],
                ],
                [
                    ...
                    'url' => app_laravel_url('cxp/windows/conciliacion-servicios-marcas'),
                    'modules' => [
                ...
                [
                    ...
                    'title' => 'Facturacion Repuestos TYTSERV',
                    'summary' => 'Ventana operativa para cargar las 4 bases RepLibroVentasGeneral y las 4 bases RepLibroDevolucionesGeneral del mes y obtener el reporte final segun la plantilla manual.',
                    ...
                    'url' => app_laravel_url('cxp/windows/facturacion-repuestos-tytserv'),
                    'modules' => [
...
function app_output_file_entries(): array
{
    ...
    usort(
        ...
        static function (string $a, string $b): int {
            $timeA = app_output_file_timestamp($a);
            $timeB = app_output_file_timestamp($b);
            if ($timeA === $timeB) {
    ...
    foreach ($files as $file) {
        ...
        $timestamp = app_output_file_timestamp($file);
        $items[] = [
            'path' => $file,
            'name' => basename($file),
...
function app_output_file_timestamp(string $file): int
{
    if (!is_file($file)) {
        return 0;
    }

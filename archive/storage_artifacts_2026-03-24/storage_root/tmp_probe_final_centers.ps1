...
foreach($path in $files){
  ...
  $centers = New-Object System.Collections.Generic.HashSet[string]
  ...
  for($r=15; $r -le $lastRow; $r++){
    ...
    if($center -ne ''){ [void]$centers.Add($center) }
    if($line -ne ''){ [void]$lines.Add($line) }
  }
  Write-Host ("{0} centers=[{1}] lines=[{2}]" -f (Split-Path $path -Leaf), (($centers.ToArray() | Sort-Object) -join ','), (($lines.ToArray() | Sort-Object) -join ','))
  $wb.Close($false)
}
$excel.Quit()

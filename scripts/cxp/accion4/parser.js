...
function validateAccountTotals(rows, accountHeaderTotals) {
  ...
  for (const row of rows) {
    if (!actualByCode.has(row.COD)) {
      actualByCode.set(row.COD, { total_debe: 0, total_haber: 0, saldo_final: 0 });
    }
  ...
  for (const [code, expected] of accountHeaderTotals.entries()) {
    const actual = actualByCode.get(code) || { total_debe: 0, total_haber: 0, saldo_final: 0 };
...
async function parseTxtRows(inputTxtPath) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (accountCodeDetected === null) {
      ...
      throw new Error(`Accion 4 requiere un solo mayor IVA por archivo. Mezcla detectada en ${path.basename(inputTxtPath)}.`);
    }
  ...
  if (skippedDateRows.length > 0) {
    ...
    throw new Error(`Se detectaron ${skippedDateRows.length} filas con fecha que no pudieron mapearse en TXT Accion 4. ${preview}`);
  }
  ...
  if (totalsValidation.mismatches.length > 0) {
    const preview = totalsValidation.mismatches
      ...
      .join(" | ");
    throw new Error(`Validacion contable fallida en TXT Accion 4 (${totalsValidation.mismatches.length} cuentas). ${preview}`);
  }
  if (accountHeaderTotals.size !== 1) {
    throw new Error(`Accion 4 solo admite un codigo contable por archivo. Se detectaron ${accountHeaderTotals.size} cuentas en ${path.basename(inputTxtPath)}.`);
  }
  ...
  if (Math.abs(round2(lastRawBalance - fileClosingBalance)) > 0.05) {
    throw new Error(`El saldo final del archivo ${path.basename(inputTxtPath)} no coincide con la ultima fila del TXT.`);
  }

...
const DEFAULT_TEMPLATE_XLSX = path.join(__dirname, "outputs", "EJEMPLOSAMANO", "MAYORIVAACCION4.xlsx");
...
function readTextFileWithBestEffortEncoding(filePath) {
...
function normalizeDocuForWorkbook(value) {
...
async function parseTxtRows(inputTxtPath) {
  const rawText = readTextFileWithBestEffortEncoding(inputTxtPath);
  const lines = rawText
    .split(/\r?\n/)
  ...
  let fileOpeningBalance = null;
  let fileClosingBalance = null;
  let accountCodeDetected = null;
  let accountNameDetected = null;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const docu = normalizeDocuForWorkbook(cols[25]);
    const detalle = sanitizeDetailText(cols[26]);
    ...
    if (fileOpeningBalance === null) {
      fileOpeningBalance = round2(parseDecimalLike(cols[8]));
    }
    fileClosingBalance = round2(parseDecimalLike(cols[11]));

    if (accountCodeDetected === null) {
      accountCodeDetected = code;
      accountNameDetected = accountName;
    } else if (accountCodeDetected !== code || accountNameDetected !== accountName) {
      throw new Error(
        `Accion 4 requiere un solo mayor IVA por archivo. Mezcla detectada en ${path.basename(inputTxtPath)}.`,
      );
    }
  ...
  if (rows.length === 0) {
    throw new Error("No se detectaron movimientos validos en el TXT de Accion 4.");
  }
  ...
  if (skippedDateRows.length > 0) {
    const preview = skippedDateRows
      ...
      .join(" | ");
    throw new Error(
      `Se detectaron ${skippedDateRows.length} filas con fecha que no pudieron mapearse en TXT Accion 4. ${preview}`,
    );
  }
  ...
  if (totalsValidation.mismatches.length > 0) {
    const preview = totalsValidation.mismatches
      ...
      .join(" | ");
    throw new Error(
      `Validacion contable fallida en TXT Accion 4 (${totalsValidation.mismatches.length} cuentas). ${preview}`,
    );
  }

  if (accountHeaderTotals.size !== 1) {
    throw new Error(
      `Accion 4 solo admite un codigo contable por archivo. Se detectaron ${accountHeaderTotals.size} cuentas en ${path.basename(inputTxtPath)}.`,
    );
  }

  const lastRawBalance = round2(rows[rows.length - 1].SALDO);
  if (fileClosingBalance === null) {
    fileClosingBalance = lastRawBalance;
  }
  if (Math.abs(round2(lastRawBalance - fileClosingBalance)) > 0.05) {
    throw new Error(
      `El saldo final del archivo ${path.basename(inputTxtPath)} no coincide con la ultima fila del TXT.`,
    );
  }

  return {
    rows,
    openingBalance: round2(fileOpeningBalance || 0),
    closingBalance: round2(fileClosingBalance || 0),
    accountCode: accountCodeDetected,
    accountName: accountNameDetected,
    diagnostics: {
      source_type: "txt",
      ...
      saldo_inicial_fuente: round2(fileOpeningBalance || 0),
      saldo_final_fuente: round2(fileClosingBalance || 0),
    },
  };
}
...
async function parsePdfRows(inputPdfPath) {
  ...
  if (rows.length === 0) {
    throw new Error("No se detectaron movimientos validos en el PDF de Accion 4.");
  }
  ...
  if (skippedDateRows.length > 0) {
    const preview = skippedDateRows
      ...
      .join(" | ");
    throw new Error(
      `Se detectaron ${skippedDateRows.length} filas con fecha que no pudieron mapearse en Accion 4. ${preview}`,
    );
  }
  ...
  if (totalsValidation.mismatches.length > 0) {
    const preview = totalsValidation.mismatches
      ...
      .join(" | ");
    throw new Error(
      `Validacion contable fallida en Accion 4 (${totalsValidation.mismatches.length} cuentas). ${preview}`,
    );
  }
...
async function parseInputSources(inputSources) {
  ...
  if (resolvedInputs.length === 0) {
    throw new Error("No se recibieron archivos de entrada para Accion 4.");
  }
  ...
  if (extensions.length !== 1) {
    throw new Error("No se pueden mezclar tipos de archivo en una sola ejecucion de Accion 4.");
  }

  const inputExt = extensions[0];
  if (inputExt !== ".txt") {
    throw new Error("Solo se permiten archivos TXT para Accion 4.");
  }

  if (resolvedInputs.length !== 1) {
    throw new Error("Accion 4 solo admite un archivo TXT por ejecucion.");
  }
  ...
  const diagnostics = {
    source_type: "txt",
    files_total: 1,
    lines_total: parsed.diagnostics.lines_total || 0,
    date_rows_detected: parsed.diagnostics.date_rows_detected || 0,
    movement_rows_extracted: parsed.rows.length,
    skipped_date_rows: parsed.diagnostics.skipped_date_rows || 0,
    account_totals_checked: parsed.diagnostics.account_totals_checked || 0,
    account_total_mismatches: parsed.diagnostics.account_total_mismatches || 0,
    saldo_inicial_consolidado: round2(parsed.openingBalance),
    saldo_final_consolidado: round2(parsed.closingBalance),
  };
    ...
    sourceFiles: [
      {
        ...
        offset_saldo_aplicado: 0,
        saldo_inicial_fuente: round2(parsed.openingBalance),
        saldo_final_fuente: round2(parsed.closingBalance),
        saldo_final_consolidado: round2(parsed.closingBalance),
        ...parsed.diagnostics,
      },
    ],
...
function cloneMovementRow(row) {
  return {
    ...
    CUENTA: row.CUENTA,
    EXT: row.EXT,
    FECHA: row.FECHA instanceof Date ? new Date(row.FECHA.getTime()) : row.FECHA,
    ORIGEN: row.ORIGEN,
    ASIENTO: row.ASIENTO,
    DOCU: row.DOCU,
    DETALLE: row.DETALLE,
...
function buildAction4OutputPlan(rows) {
  ...
  let agcmCount = 0;
  while (agcmCount < normalizedRows.length && sanitizeText(normalizedRows[agcmCount].ORIGEN).toUpperCase() === "AGCM") {
    agcmCount += 1;
  }
...
async function buildWorkbookFromTemplate(templatePath, rowPlan, movementRows) {
  ...
  const requiredRowCount = Math.max(rowPlan.length + 1, ws.rowCount);
  ...
  for (let i = 0; i < rowPlan.length; i += 1) {
    ...
    if (planItem.type === "movement") {
      ...
      ws.getCell(rowNum, 9).value = {
        formula: `SUM(I2:I${planItem.sumToRow})`,
        result: planItem.debe,
      };
      ws.getCell(rowNum, 10).value = {
        formula: `SUM(J2:J${planItem.sumToRow})`,
        result: planItem.haber,
      };
    } else if (planItem.type === "subtotal_balance") {
      ws.getCell(rowNum, 11).value = {
        formula: `+I${rowNum - 1}-J${rowNum - 1}`,
        result: planItem.saldo,
      };
    }
...
async function writeWorkbookWithRetries(workbook, preferredPath, maxAttempts = 20) {
  ...
  throw new Error("No se pudo guardar el Excel de Accion 4. Cierra archivos abiertos e intenta de nuevo.");
}
